...
s = a0*f**(-n0) + C0 + P0*np.exp(-0.5*(((np.log(f))-fp0)/fw0)**2)
...
scoeff = Slider(axcoeff, 'Coeff', 0.0000001, 0.0001, valinit=a0)
sindex = Slider(axindex, 'Index', 0.3, 4.0, valinit=n0)
stail = Slider(axtail, 'Tail', 0.000001, 0.03, valinit=C0)
samp = Slider(axamp, 'Amp', 0.00001, 0.1, valinit=P0)
...
def update(val):
    ...
    l.set_ydata(coeff*f**(-index)+tail + amp*np.exp(-0.5*(((np.log(f))-(loc))/wid)**2))
