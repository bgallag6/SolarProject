...
for loc in range(2,8):
    for widb in [0,1,2]:
        ...
        #plt.vlines((1./600.),0.,1., linestyle='solid')
        #plt.hlines(P0/2.,0.002,0.01)
        plt.title('Location = %i Minutes | Width = %0.2e' % (loc,wid[widb]))
        #plt.plot(f, s, lw=2, color='red')
        #plt.plot(f, m2L, 'g')
        #plt.plot(f, m2)
        #plt.ylim(0.,0.02)
        ...
        print(widthp, (1./val_l) - (1./val_r), b_freq)
        fwhm = (1./val_l) - (1./val_r)
        
        arr[loc-2][widb] = fwhm
        
        bleft = b_freq-widthp
        bright = b_freq+widthp
        pleft = 1./bleft
        pright = 1./bright
        #print(pleft-pright)
        ...
        #arr2[loc-2][widb] = pleft-pright
        arr2[loc-2][widb] = (1./(b_freq-widthp)) - (1./(b_freq+widthp))  # wrong
        """
        plt.figure()
