# -*- coding: utf-8 -*-
"""
Created on Fri Feb 23 17:23:13 2018
...
def datacube(flist_chunk):
    ...
    np.save('%s/DATA/Temp/%s/%i/chunk_%i_of_%i' % (directory, date, wavelength, rank, size), dCube_trim)
...
if rank == 0:
  ...
  np.save('%s/DATA/Temp/%s/%i/exposure.npy' % (directory, date, wavelength), ex_arr)
  np.save('%s/DATA/Temp/%s/%i/time.npy' % (directory, date, wavelength), t_arr)
  np.save('%s/DATA/Output/%s/%i/visual.npy' % (directory, date, wavelength), v_avg_arr)
...
"""
## I'm pretty sure MPI can't handle chunks as big as 1/16th of a derotated datacube, so can't use below

all_chunks = comm.gather(cube_chunk, root=0)  # Gather all the results

# Have one node stack the results
if rank == 0:
  dcube_final = np.vstack(all_chunks)
  print(dcube_final.shape, flush=True)  # Verify we have a summed version of the input cube
  ...
  np.save('%s/DATA/Temp/%s/%i/derotated.npy'% (directory, date, wavelength), dcube_final)
"""