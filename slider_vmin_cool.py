# -*- coding: utf-8 -*-
"""
Created on Thu Dec 22 09:28:21 2016
...
im, = ([ax1.imshow(R, cmap='jet', interpolation='nearest', picker=True)])  # gauss amp
...
axcolor = 'white'
ax_vmin = plt.axes([0.05, 0.05, 0.45, 0.04], axisbg=axcolor)
...
def update(val):
    ...
    for i in range(R.shape[0]):
        for j in range(R.shape[1]):
            if R[i][j] < vmin:
                R[i][j] = np.nan
    ax1.imshow(R, cmap='jet', interpolation='nearest', picker=True) 
